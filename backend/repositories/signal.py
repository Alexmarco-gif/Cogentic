...
class SignalRepository(BaseRepository[Signal]):
    ...
    async def get_by_ids_scoped(
        self, ids: list[UUID], *, org_id: UUID | None = None
    ) -> list[Signal]:
        """Get multiple signals by ID with org-scoping.

        Returns global signals AND signals belonging to the given org.
        Use this instead of BaseRepository.get_by_ids for tenant-safe bulk fetches.
        """
        if not ids:
            return []
