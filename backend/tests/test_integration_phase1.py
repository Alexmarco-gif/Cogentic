...
def mock_jwks():
    ...
    with patch("backend.auth.utils.jwt.decode") as mock_decode:
        ...
        def decode_side_effect(token, *args, **kwargs):
            ...
            from jose import jwt
            return jwt.decode(token, TEST_SECRET_KEY, algorithms=["HS256"], options={"verify_signature": False})
...
class TestOrganizationIsolation:
    ...
    def test_user_in_multiple_orgs(self, client, mock_jwks, seed_test_data):
        ...
        response2 = client.get(
            f"/api/v1/orgs/{TEST_ORG_2_ID}",
            headers=auth_headers(token_org2)
        )
    ...
    def test_org_member_list_isolated(self, client, mock_jwks, seed_test_data):
        ...
        response = client.get(
            f"/api/v1/orgs/{TEST_ORG_1_ID}/members",
            headers=auth_headers(token)
        )
...
class TestRoleBasedPermissions:
    ...
    def test_viewer_has_read_only_access(self, client, mock_jwks, seed_test_data, test_db):
...
class TestAPIKeyAuthentication:
    ...
    def test_cannot_create_more_than_max_keys(self, client, mock_jwks, seed_test_data):
        ...
        with patch("backend.repositories.api_key.APIKeyRepository.count_active_by_org") as mock_count:
...
class TestRateLimiting:
    ...
    def test_authenticated_higher_rate_limit(self, client, mock_jwks, mock_redis, seed_test_data):
...
class TestFeatureFlagsWithAuth:
    ...
    def test_check_specific_feature_flag(self, client, mock_jwks, seed_test_data):
        ...
        response = client.get(
            "/api/v1/features/advanced_analytics",
            headers=auth_headers(token)
        )
