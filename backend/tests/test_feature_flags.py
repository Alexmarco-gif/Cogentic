...
class TestFeatureFlagService:
    ...
    def test_user_override(self, feature_service):
        ...
        assert (
            feature_service.is_enabled("test_user_override", user_id="user-123") is True
        )

        # User not in allowlist cannot access
        assert (
            feature_service.is_enabled("test_user_override", user_id="user-999")
            is False
        )
    ...
    def test_org_override(self, feature_service):
        ...
        assert feature_service.is_enabled("test_org_override", org_id="org-abc") is True

        # Org not in allowlist cannot access
        assert (
            feature_service.is_enabled("test_org_override", org_id="org-999") is False
        )
...
class TestFeatureGuards:
    ...
    def create_auth_context(
        self, user_id=None, org_id=None, plan="free", role="member"
    ):
    ...
    def test_require_feature_enabled(self, feature_service, monkeypatch):
        ...
        monkeypatch.setattr(
            "backend.services.feature_flags._feature_flags_service", feature_service
        )
    ...
    def test_require_feature_with_user_override(self, feature_service, monkeypatch):
        ...
        feature_service.features["test_user_override"].enabled_for_users.append(
            str(user_uuid)
        )
    ...
    def test_require_feature_with_org_override(self, feature_service, monkeypatch):
        ...
        feature_service.features["test_org_override"].enabled_for_orgs.append(
            str(org_uuid)
        )
...
class TestFeatureDefinition:
    ...
    def test_basic_definition(self):
        ...
        feature = FeatureDefinition(enabled=True, description="Test feature")
        assert feature.enabled is True
    ...
    def test_definition_with_plan(self):
        ...
        feature = FeatureDefinition(
            enabled=True, description="Pro feature", required_plan="pro"
        )
