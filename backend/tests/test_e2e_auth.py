...
class TestUnauthenticatedAccess:
    ...
    def test_error_message_indicates_auth_needed(self, client):
        ...
        assert any(
            word in error_text
            for word in ["unauthorized", "authentication", "token", "bearer", "auth"]
        )
...
class TestInvalidTokens:
    ...
    def test_expired_looking_token_rejected(self, client):
        ...
        response = client.get(
            "/api/v1/auth/me", headers={"Authorization": f"Bearer {expired_jwt}"}
        )
...
class TestAuthHeaders:
    ...
    def test_case_insensitive_authorization_header(
        self, client, auth_token, requires_auth
    ):
        """
...
class TestHealthAuth:
    ...
    def test_authenticated_health_endpoint(
        self, authed_client, auth_token, requires_auth
    ):
        """
