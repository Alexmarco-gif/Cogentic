...
class TestWebhookEndpoints:
    ...
    def test_auth0_webhook_endpoint_exists(self, client):
        ...
        """
        response = client.post("/webhooks/auth0/events", json={"test": "data"})
...
class TestWebhookSecurity:
    ...
    def test_webhook_rejects_tampered_payload(self, client):
        ...
        signature = hmac.new(
            test_secret.encode(), json.dumps(original_payload).encode(), hashlib.sha256
        ).hexdigest()
...
class TestWebhookEventTypes:
    ...
    def test_malformed_event_data(self, client):
        ...
        response = client.post(
            "/webhooks/auth0/events",
            json={"event": "user.created", "data": "should-be-an-object-not-string"},
            headers={"X-Auth0-Signature-256": "sha256=test"},
        )
