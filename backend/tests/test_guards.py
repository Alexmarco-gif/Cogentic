...
def create_auth_context(
    role: str = "member", org_id: UUID | None = None
) -> AuthContext:
...
class MockResource:
    ...
    def __init__(
        self, owner_id: UUID, visibility: str = "private", shared_with: list = None
    ):
        self.id = uuid4()
...
class TestResourcePermissions:
    ...
    def test_can_view_shared_resource(self):
        ...
        shared_resource = MockResource(
            uuid4(), "shared", shared_with=[str(auth.user_id)]
        )
        not_shared_resource = MockResource(uuid4(), "shared", shared_with=[])
