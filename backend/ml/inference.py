...
class OnnxInferenceEngine:
    """ML inference engine with model version management.
    ...
    Model naming convention:
        {model_name}/v{version}/{model_name}.onnx   (ONNX models)
        {model_name}/v{version}/{model_name}.joblib  (sklearn joblib models)
        e.g. anomaly_detector/v1/anomaly_detector.joblib
    """
    ...
    def __init__(self):
        ...
        self._joblib_models: dict[str, Any] = {}
        ...
        *,
        ...
        try:
            ...
            if model_path.suffix == ".joblib":
                result = self._predict_joblib(model_name, version, features, model_path)
            else:
    ...
    def _predict_joblib(
        ...
        """Run prediction using a joblib-serialized sklearn model."""
        key = f"{model_name}:{version or 'latest'}"

        if key not in self._joblib_models:
            import joblib

            logger.info(f"Loading joblib model: {model_path}")
            self._joblib_models[key] = joblib.load(model_path)
            self._model_metadata[key] = {
                "path": str(model_path),
                "format": "joblib",
            }

        model = self._joblib_models[key]
    ...
    def get_loaded_models(self) -> list[str]:
        ...
        return list(set(list(self._sessions.keys()) + list(self._joblib_models.keys())))

    def get_available_versions(self, model_name: str) -> list[str]:
        """List available versions for a model (sorted descending).

        Detects both .onnx and .joblib model files.
        """
        ...
        for d in model_dir.iterdir():
            if d.is_dir() and d.name.startswith("v"):
                ...
                joblib_file = d / f"{model_name}.joblib"
                if onnx_file.exists() or joblib_file.exists():
    ...
    def unload_model(self, model_name: str, version: str | None = None):
        ...
        if key in self._joblib_models:
            del self._joblib_models[key]
            logger.info(f"Unloaded joblib model: {key}")
    ...
    def _validate_required_models(self) -> None:
        ...
        for model_name in settings.ml_models_required:
            try:
                model_path = self._resolve_model_path(model_name, version=None)
                if model_path.suffix == ".joblib":
                    # Validate joblib model loads
                    key = f"{model_name}:latest"
                    if key not in self._joblib_models:
                        import joblib
                    ...
                    self._get_session(model_name, version=None)
    ...
    def _get_session(
        ...
        if model_path.suffix == ".joblib":
            # For joblib models, return a dummy — actual loading is in _predict_joblib
            # But we still need to register the session key for validation
            import joblib

            logger.info(f"Loading joblib model: {model_path}")
            loaded = joblib.load(model_path)
            self._joblib_models[key] = loaded
            self._model_metadata[key] = {
                "path": str(model_path),
                "format": "joblib",
            }
            # Return a sentinel — callers should use predict() not _get_session()
            # for joblib models. This path is hit during validation.
    ...
    def _resolve_model_path(
        ...
        if version:
            # Check for .onnx first, then .joblib
            onnx_path = model_dir / version / f"{model_name}.onnx"
            joblib_path = model_dir / version / f"{model_name}.joblib"
            if onnx_path.exists():
                return onnx_path
            if joblib_path.exists():
                return joblib_path
            raise FileNotFoundError(f"Model not found: {onnx_path} or {joblib_path}")

        # Find latest version
        versions = self.get_available_versions(model_name)
        if not versions:
            raise FileNotFoundError(
                f"No versions found for model: {model_name} (searched: {model_dir})"
            )

        latest = versions[0]
        onnx_path = model_dir / latest / f"{model_name}.onnx"
        if onnx_path.exists():
            return onnx_path
        return model_dir / latest / f"{model_name}.joblib"
