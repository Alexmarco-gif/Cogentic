...
async def get_current_user(
    ...
    if not user:
        ...
        email = payload.sub.split("|")[-1] if "|" in payload.sub else f"{payload.sub}@unknown.com"
...
async def get_current_user_or_api_key(
    ...
    if not user:
        logger.error(f"API key creator user {api_key_model.created_by_user_id} not found")
    ...
    if not org_user:
        logger.error(
            f"API key user {user.id} not member of org {api_key_model.org_id}"
        )
