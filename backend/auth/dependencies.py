...
async def get_current_user(
    ...
    # Record session in background — non-blocking, best-effort
...
async def _upsert_session_bg(
    user_id: UUID,
    ip_address: str,
    user_agent: str | None,
) -> None:
    """Fire-and-forget session upsert using its own DB session."""
    from backend.database import AsyncSessionLocal  # avoid circular at module level

    try:
        async with AsyncSessionLocal() as db:
            repo = UserSessionRepository(db)
            await repo.upsert_session(user_id, ip_address, user_agent)
    except Exception:
        logger.exception(
            "Background session upsert failed",
...
async def _handle_user_token(
    ...
    if not user:
        ...
        # Email is required — tokens without a verified email claim are rejected
        # to prevent placeholder addresses from being stored in the DB.
        if not payload.email:
            logger.error(
                f"Token for subject {payload.sub} is missing the email claim. "
                "Auth0 must be configured to include the email in the token."
            )
            raise HTTPException(
    ...
    # Session tracking is handled by get_current_user via background task
    # (see _upsert_session_bg) — no inline upsert needed here.
...
async def get_optional_user(
    ...
    Args:
        request: FastAPI request
        background_tasks: BackgroundTasks for session tracking
        db: Database session
    ...
    """
    try:
        return await get_current_user(request, background_tasks, db)
    except (MissingTokenError, InvalidTokenError, NotOrgMemberError, HTTPException):
        # No token, invalid token, or failed org check — return None for optional auth
        return None
...
async def get_current_user_or_api_key(
    ...
    # and use the org's actual pricing tier for plan context
    org = await org_repo.get(api_key_model.org_id)
    org_plan = org.pricing_tier if org else "explorer"
