...
TOOL_SCHEMAS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Create a new signal contract to actively track and acquire signals from a "
                "specific source URL. Creates a fully functional contract that the signal "
                "acquisition system will start fetching from. Use when the user wants to "
                "monitor a new data source, website, API, RSS feed, or social media topic."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "name": {
                        "type": "string",
                        "description": "Name of the signal contract (e.g., 'CBN Policy Updates').",
                    },
                    "description": {
                        "type": "string",
                        "description": "What this contract tracks and why it matters.",
                    },
                    "source_url": {
                        "type": "string",
                        "description": "The URL to fetch signals from (website, API endpoint, RSS feed URL).",
                    },
                    "source_type": {
                        "type": "string",
                        "enum": ["api", "scraper", "rss", "social"],
                        "description": "How to fetch: api (JSON APIs), scraper (HTML pages), rss (RSS/Atom feeds), social (social media).",
                    },
                    "signal_type": {
                        "type": "string",
                        "enum": [
                        ...
                        "description": "Type of signal this source produces.",
                    },
                    "schedule_tier": {
                        "type": "string",
                        "enum": ["realtime", "standard", "slow", "daily"],
                        "description": "How often to fetch: realtime (15min), standard (1hr), slow (6hr), daily. Defaults to standard.",
                    },
                    "industry": {
                        "type": "string",
                        "description": "Industry context (e.g., 'fintech', 'fmcg', 'agriculture').",
                    },
                },
                "required": ["name", "source_url", "source_type", "signal_type"],
            },
        },
    },
]
...
async def execute_deep_search(
    ...
    *,
    ...
    simplified_results = []
    for r in result.get("signals", [])[:max_results]:
    ...
    # Include raw web results for citation extraction
    web_results = []
    for wr in result.get("web_results", [])[:10]:
        web_results.append(
            {
                "title": wr.get("title", ""),
                "snippet": (wr.get("snippet") or "")[:300],
                "url": wr.get("url"),
...
async def execute_synthesize_signal(
    ...
    *,
    ...
    return {
        ...
        "sources_used": result.get("source_count", 0),
        "evidence": [
            {
                "title": e.get("title", ""),
...
async def execute_create_contract(
    ...
    *,
    ...
    """Create a new functional signal contract.

    Creates a real contract with source_url, extraction config, and schedule —
    the acquisition pipeline will pick it up and start fetching.
    """
    # Map schedule tier to cron expression
    tier = schedule_tier or "standard"
    cron_map = {
    ...
    refresh_cron = cron_map.get(tier, cron_map["standard"])
    ...
    if source_type == "rss":
        extraction_config["parser"] = "rss"
        extraction_config["max_items"] = 20
    elif source_type == "api":
        extraction_config["parser"] = "json"
        extraction_config["response_format"] = "json"
    elif source_type == "scraper":
        extraction_config["parser"] = "html"
        extraction_config["selectors"] = {
            "title": "h1, .article-title, .headline",
            "content": "article, .content, .article-body, main",
            "date": "time, .date, .published",
        }
    elif source_type == "social":
        extraction_config["parser"] = "social"

    # Record intended signal type so the acquirer tags signals correctly
    if signal_type:
        extraction_config["signal_type"] = signal_type

    # Resolve industry to industry_id
    industry_id = None
    if industry:
        ...
        result = await db.execute(
            select(Industry.id).where(Industry.code == industry).limit(1)
        )
        row = result.scalar_one_or_none()
        if row:
            industry_id = row
    # Fallback: use first industry if not resolved
    if not industry_id:
        from sqlalchemy import select

        from backend.models.industry import Industry

        result = await db.execute(select(Industry.id).limit(1))
        industry_id = result.scalar_one_or_none()

    contract = SignalContract(
        name=name,
        description=description or f"Signal contract for {name}",
    ...
    logger.info(
        f"Chat agent created functional contract {contract.id}: "
        f"{name} ({source_type} → {source_url})"
    )
    ...
    return {
        ...
        "result": {
            ...
            "industry": industry,
            "status": "active",
            "message": f"Contract created and active. The system will start fetching from {source_url} on a {tier} schedule.",
        },
    }
