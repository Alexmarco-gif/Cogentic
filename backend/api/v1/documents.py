...
def _doc_response(doc: Any, auth: AuthContext) -> DocumentResponse:
    """Build DocumentResponse, redacting storage_path for non-admin users."""
    show_path = Role.from_string(auth.role) >= Role.ADMIN
    return DocumentResponse(
        id=str(doc.id),
        filename=doc.filename,
        storage_path=doc.storage_path if show_path else None,
...
async def list_documents(
    ...
    if owner_id:
        ...
        documents = await repo.get_multi(skip=skip, limit=limit, filters={"deleted_at": None})
        total = await repo.count(filters={"deleted_at": None})

    return {
        "documents": [
            _doc_response(doc, auth)
...
async def create_document(
    ...
    auth: AuthContext = Depends(get_current_user),
    _feature_gate: bool = Depends(require_feature("bulk_document_operations")),
    db: AsyncSession = Depends(get_db),
) -> DocumentResponse:
    """
    ...
    Feature gate: bulk_document_operations (enforced via dependency)
    """
    require_org_membership(auth, org_id)
