...
async def extract_regulatory_event_from_signal(
    signal_id: UUID,
    auto_create: bool = Query(True, description="Automatically create event if detected"),
    db: AsyncSession = Depends(get_db),
...
async def get_regulatory_knowledge_stats(
    ...
    total_impacts = await db.execute(
        select(func.count(RegulatoryImpact.id))
    )
    ...
    return {
        ...
        "events_by_regulator": [
            {"regulator": row[0], "count": row[1]}
            for row in events_by_body
        ],
...
class PatternLearningRequest(BaseModel):
    ...
    lookback_months: int = Field(36, ge=1, le=120, description="Months of history to analyze")
    min_occurrences: int = Field(3, ge=2, le=10, description="Minimum pattern occurrences")
