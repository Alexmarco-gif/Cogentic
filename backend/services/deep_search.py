...
class DeepSearchService:
    ...
    Pipeline:
      ...
      4. Parallel fetch: pgvector signals + entity matches + SerpApi live web
      5. Semantic ranking via embedding similarity + confidence + freshness
      6. Result dedup and fusion (internal signals + live web results)
      7. Optional AI synthesis of top results (enriched with web context)
    ...
    def __init__(self, db: AsyncSession):
        ...
        # Live web search provider (SerpApi by default)
        from backend.services.web_search import get_web_search_provider

        self._web_search = get_web_search_provider()
    ...
    async def search(
        ...
        *,
        ...
        # Step 4: Parallel fetch — signals + entities + live web
        ...
        signals, entities, web_results = await asyncio.gather(
            signal_task, entity_task, web_task, return_exceptions=True
        )
        ...
        # Step 5: Fuse internal signals with live web results, then rank
        fused = self._fuse_results(signals, web_results)
        ranked_signals = self._rank_results(fused)

        # Step 6: Optional AI synthesis (with both internal + web context)
        ...
        if synthesize and ranked_signals:
            try:
                synthesis_result = await self.synthesis.synthesize(
                    ...
                    web_context=web_results if web_results else None,
                )
            except Exception as e:
    ...
    async def _search_web(
        ...
        *,
        ...
        Returns a list of WebSearchResult objects. Degrades gracefully
        if the provider is unavailable or returns errors.
        """
        from backend.services.web_search.base import WebSearchError

        try:
            if not await self._web_search.is_available():
            ...
            google_results, news_results = await asyncio.gather(
                google_task, news_task, return_exceptions=True
            )
            ...
            elif isinstance(google_results, WebSearchError):
                logger.warning(f"Google web search error: {google_results.message}")

            if isinstance(news_results, list):
                all_results.extend(news_results)
            elif isinstance(news_results, WebSearchError):
                logger.warning(f"Google News search error: {news_results.message}")
            ...
            logger.info(
                f"Web search returned {len(deduped)} results "
                f"(google={len(google_results) if isinstance(google_results, list) else 0}, "
                f"news={len(news_results) if isinstance(news_results, list) else 0})"
            )
            return deduped
    ...
    def _fuse_results(
        ...
        """Fuse internal pgvector signals with live web results.

        Web results are converted to signal-compatible dicts and
        interleaved with internal signals. Internal signals get a
        slight boost since they come from our curated pipeline.
        """
        fused = list(internal_signals)  # Copy

        for wr in web_results:
            if not hasattr(wr, "to_dict"):
                continue

            # Convert WebSearchResult → signal-compatible dict
            fused.append(
                {
                    "id": None,  # No DB ID — live web result
                    "title": wr.title or "Untitled",
                    "summary": wr.snippet or "",
                    "confidence": wr.confidence,
                    "signal_type": "web",
                    "source_url": wr.url,
                    "published_at": (
                        wr.published_at.isoformat() if wr.published_at else None
                    ),
                    "similarity": wr.relevance_score or 0.60,
                    "freshness_score": 0.90,  # Web results are inherently fresh
                    "source": wr.source,
                    "is_live_web": True,
