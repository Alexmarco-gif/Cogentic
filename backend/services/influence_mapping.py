...
class InfluenceMappingService:
    ...
    async def calculate_entity_influence_score(
        ...
        *,
        ...
        try:
            eigenvector_scores = nx.eigenvector_centrality(graph, weight="weight", max_iter=1000)
            eigenvector_score = eigenvector_scores.get(node_id, 0.0)
        except nx.PowerIterationFailedConvergence:
            logger.warning(f"Eigenvector centrality failed to converge for entity {entity_id}")
            eigenvector_score = 0.0
        
        # Composite score (weighted average)
        composite_score = (
            pagerank_score * 0.30 +      # Global importance
            betweenness_score * 0.25 +   # Broker position
            eigenvector_score * 0.20 +   # Connected to influencers
            degree_score * 0.15 +        # Direct connections
            closeness_score * 0.10       # Network access
        )
        ...
        *,
        ...
        for entity_id_str, score in ranked_entities:
            ...
            if entity:
                ...
                metrics = await self.calculate_entity_influence_score(entity_id, industry_id=industry_id)
    ...
    async def predict_influence_cascade(
        ...
        *,
        ...
        while queue:
            ...
            for neighbor in graph.successors(current_id):
                if neighbor not in visited:
                    ...
                    propagated_influence = current_influence * edge_strength * propagation_decay
        ...
        return {
            ...
            "max_depth_reached": max(e["cascade_depth"] for e in sorted_affected) if sorted_affected else 0,
    ...
    async def _build_network_graph(
        ...
        *,
        ...
        for rel, entity in relationships:
            ...
            relationship_weight = self.RELATIONSHIP_WEIGHTS.get(rel.relationship_type, 0.5)
        ...
        logger.info(f"Built network graph with {graph.number_of_nodes()} nodes and {graph.number_of_edges()} edges")
