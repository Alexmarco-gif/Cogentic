...
class FeatureDefinition(BaseModel):
    ...
    required_plan: Literal["free", "pro", "enterprise"] | None = Field(
        None,
        description="Minimum plan required (None = available to all)"
    )
        ...
        try:
            ...
            for feature_name, feature_data in data["features"].items():
                try:
                    ...
                    logger.error(
                        f"Failed to parse feature '{feature_name}': {e}",
                        exc_info=True
                    )
            
            logger.info(f"Loaded {len(self.features)} feature flags from {self.config_path}")
    ...
    def is_enabled(
        ...
        *,
        ...
        if feature.required_plan:
            ...
            has_required_plan = plan_hierarchy.get(plan, 0) >= plan_hierarchy.get(feature.required_plan, 0)
