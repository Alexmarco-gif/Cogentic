...
def upgrade() -> None:
    ...
    has_entities = _table_exists(inspector, "entities")
    has_signals = _table_exists(inspector, "signals")
    has_users = _table_exists(inspector, "users")
    has_organizations = _table_exists(inspector, "organizations")
    has_causal_events = _table_exists(inspector, "causal_events")

    # ── 1. Add verified column to entities ────────────────────────────
    if has_entities and not _column_exists(
        inspector, "entities", "verified"
    ):
        op.add_column(
            "entities",
            sa.Column(
    ...
    # ── 2. Entity Aliases ─────────────────────────────────────────────
    if has_entities and not _table_exists(inspector, "entity_aliases"):
        op.create_table(
            "entity_aliases",
        ...
        op.create_index(
            "ix_entity_aliases_entity_id", "entity_aliases", ["entity_id"]
        )
        op.create_index(
            "ix_entity_aliases_alias_name", "entity_aliases", ["alias_name"]
        )

    # ── 3. Entity Source Profiles ─────────────────────────────────────
    if has_entities and not _table_exists(inspector, "entity_source_profiles"):
        op.create_table(
        ...
        op.create_index(
            "ix_entity_source_profiles_entity_id",
            "entity_source_profiles",
            ["entity_id"],
        )
        op.create_index(
    ...
    if has_entities and not _table_exists(inspector, "entity_relationships"):
        op.create_table(
    ...
    # ── 5. Causal Events ──────────────────────────────────────────────
    if has_signals and not has_causal_events:
        op.create_table(
            "causal_events",
            sa.Column("id", UUID(as_uuid=True), primary_key=True),
            sa.Column(
        ...
        op.create_index(
            "ix_causal_events_signal_id", "causal_events", ["signal_id"]
        )
        op.create_index(
            "ix_causal_events_event_type", "causal_events", ["event_type"]
        )
        op.create_index(
            "ix_causal_events_category", "causal_events", ["event_category"]
        )
        op.create_index(
            "ix_causal_events_timestamp", "causal_events", ["event_timestamp"]
        )
        has_causal_events = True

    # ── 6. Causal Edges ───────────────────────────────────────────────
    if has_causal_events and not _table_exists(inspector, "causal_edges"):
        op.create_table(
            "causal_edges",
            sa.Column("id", UUID(as_uuid=True), primary_key=True),
            sa.Column(
            ...
            sa.Column(
                "lag_days_min", sa.Integer(), nullable=False, server_default="0"
            ),
            sa.Column(
                "lag_days_max", sa.Integer(), nullable=False, server_default="0"
            ),
        ...
        op.create_index(
            "ix_causal_edges_cause", "causal_edges", ["cause_event_id"]
        )
        op.create_index(
            "ix_causal_edges_effect", "causal_edges", ["effect_event_id"]
        )
...
def downgrade() -> None:
    ...
    if _table_exists(inspector, "user_feedback"):
        op.drop_table("user_feedback")
    if _table_exists(inspector, "causal_edges"):
        op.drop_table("causal_edges")
    if _table_exists(inspector, "causal_events"):
        op.drop_table("causal_events")
    if _table_exists(inspector, "entity_relationships"):
        op.drop_table("entity_relationships")
    if _table_exists(inspector, "entity_source_profiles"):
        op.drop_table("entity_source_profiles")
    if _table_exists(inspector, "entity_aliases"):
        op.drop_table("entity_aliases")
    if _table_exists(inspector, "entities") and _column_exists(inspector, "entities", "verified"):
