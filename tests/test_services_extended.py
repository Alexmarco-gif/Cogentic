...
- PricingService: subscription pricing, beta discount, overage
- TrialService: trial lifecycle (start, expiry, conversion)
- FeatureFlagService: YAML-based feature flags
- BetaLifecycleService: beta account management
...
# ---------------------------------------------------------------------------
# Helper: create a BetaAccount row
# ---------------------------------------------------------------------------
async def create_beta_account(
    ...
    *,
    ...
    ba = BetaAccount(
        id=uuid4(),
        org_id=org_id,
        beta_start_date=beta_start_date or now - timedelta(days=30),
        beta_end_date=beta_end_date or now + timedelta(days=30),
        discount_percent=discount_percent,
        transitioned_to_standard=transitioned,
        notified_14d_before=notified_14d,
        notified_7d_before=notified_7d,
    )
    db.add(ba)
    await db.flush()
    await db.refresh(ba)
...
class TestPricingService:
    ...
    async def test_calculate_subscription_price_with_beta_discount(
        self, db_session: AsyncSession
    ):
        """Beta discount is applied correctly."""
        await self._seed_tier_price(db_session, "growth", 200)
        org = await create_organization(db_session, pricing_tier="growth")
        org.is_beta_account = True
        org.beta_end_date = datetime.now(timezone.utc) + timedelta(days=30)
        org.beta_discount_percent = 50.0
        await db_session.flush()

        svc = PricingService(db_session)
        price = await svc.calculate_subscription_price(org)
        assert price == Decimal("100")  # 50% off 200

    async def test_calculate_subscription_price_expired_beta(
        self, db_session: AsyncSession
    ):
        """Expired beta does not get discount."""
        await self._seed_tier_price(db_session, "growth", 200)
        org = await create_organization(db_session, pricing_tier="growth")
        org.is_beta_account = True
        org.beta_end_date = datetime.now(timezone.utc) - timedelta(days=1)
        org.beta_discount_percent = 50.0
        await db_session.flush()

        svc = PricingService(db_session)
        price = await svc.calculate_subscription_price(org)
        assert price == Decimal("200")  # no discount

    async def test_is_beta_active_not_beta(self, db_session: AsyncSession):
        """Non-beta org returns False."""
        org = await create_organization(db_session)
        svc = PricingService(db_session)
        assert svc._is_beta_active(org) is False

    async def test_is_beta_active_no_end_date(self, db_session: AsyncSession):
        """Beta org without end date returns False."""
        org = await create_organization(db_session)
        org.is_beta_account = True
    ...
    async def test_get_pricing_summary(self, db_session: AsyncSession):
        ...
        assert summary["is_beta"] is False
...
# =====================================================================
#  BetaLifecycleService Tests (limited — avoids external calls)
# =====================================================================
class TestBetaLifecycleService:
    """Tests for beta account lifecycle management."""

    async def test_create_beta_account(self, db_session: AsyncSession):
        """Creating a beta account sets correct org fields."""
        from backend.services.beta_lifecycle_service import BetaLifecycleService

        org = await create_organization(db_session)
        svc = BetaLifecycleService(db_session)

        result = await svc.create_beta_account(
            org_id=org.id,
            duration_days=90,
            discount_percent=25.0,
        )
        assert result.is_beta_account is True
        assert result.beta_discount_percent == 25.0
        assert result.beta_end_date is not None

    async def test_create_beta_account_org_not_found(self, db_session: AsyncSession):
        """Raises ValueError for unknown org id."""
        from backend.services.beta_lifecycle_service import BetaLifecycleService

        svc = BetaLifecycleService(db_session)

        with pytest.raises(ValueError, match="Organization not found"):
            await svc.create_beta_account(
                org_id=uuid4(),
                duration_days=30,
            )

    async def test_extend_beta_period(self, db_session: AsyncSession):
        """Extend beta period adds days to end date."""
        from backend.services.beta_lifecycle_service import BetaLifecycleService

        org = await create_organization(db_session)
        original_end = datetime.now(timezone.utc) + timedelta(days=30)
        await create_beta_account(db_session, org.id, beta_end_date=original_end)
        # Need to also set org fields for backward compat in service
        org.is_beta_account = True
        org.beta_end_date = original_end
        await db_session.flush()

        svc = BetaLifecycleService(db_session)
        await svc.extend_beta_period(org.id, additional_days=15)

        # Verify the beta account end date was extended
        from backend.repositories.beta_repository import BetaRepository

        beta_repo = BetaRepository(db_session)
        beta = await beta_repo.get_by_org_id(org.id)
        assert beta is not None
        # Should be about 45 days from now
        days_remaining = (
            beta.beta_end_date.replace(tzinfo=None)
            - datetime.now(timezone.utc).replace(tzinfo=None)
        ).days
        assert 44 <= days_remaining <= 46

    async def test_extend_beta_no_account(self, db_session: AsyncSession):
        """Raises ValueError when no beta account exists."""
        from backend.services.beta_lifecycle_service import BetaLifecycleService

        svc = BetaLifecycleService(db_session)

        with pytest.raises(ValueError, match="No beta account found"):
            await svc.extend_beta_period(uuid4(), additional_days=10)
