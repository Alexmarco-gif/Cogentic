...
# ---------------------------------------------------------------------------
# Ensure required env vars are set BEFORE any application module is imported.
# database.py calls create_async_engine at import time, so DATABASE_URL must
# exist even though conftest later overrides the session with SQLite in-memory.
# ---------------------------------------------------------------------------
os.environ.setdefault("DATABASE_URL", "postgresql+asyncpg://test:test@localhost:5432/test")
os.environ.setdefault("SECRET_KEY", "test-secret-key-32-characters-xx")
os.environ.setdefault("AUTH0_DOMAIN", "test.auth0.com")
os.environ.setdefault("AUTH0_AUDIENCE", "test-api")
...
from backend.database import get_db, get_db_read  # noqa: E402
...
async def app(db_session: AsyncSession):
    ...
    _org = Organization(
        id=_test_org_id,
        name="Test Org",
        slug=f"test-org-{_test_org_id.hex[:8]}",
        pricing_tier="explorer",
        credits_allocated_monthly=1000,
        credits_consumed=0,
    )
    db_session.add(_org)
    _usr = User(
        id=_test_user_id,
        auth0_id="auth0|test-user-000000",
    ...
    _fake_token_payload = TokenPayload(
        ...
        gty=None,
        **{
            "https://cogent.ai/claims/org_id": str(_test_org_id),
            "https://cogent.ai/claims/user_id": str(_test_user_id),
            "https://cogent.ai/claims/email": "test@cogent.ai",
            "https://cogent.ai/claims/role": "member",
            "https://cogent.ai/claims/roles": ["member"],
            "https://cogent.ai/claims/plan": "explorer",
    ...
    # Override DB dependencies (both read-write and read-replica)
    async def _override_get_db():
        yield db_session

    _app.dependency_overrides[get_db] = _override_get_db
    _app.dependency_overrides[get_db_read] = _override_get_db
